...
Returns an array of script info objects.

| Field | Type | Description |
|-------|------|-------------|
| [].src | string | The `src` attribute, empty for inline scripts |
| [].type | string | The `type` attribute (e.g. `module`, `text/javascript`) |
| [].async | boolean | Whether the script has the `async` attribute |
| [].defer | boolean | Whether the script has the `defer` attribute |
...
```
hubcap scripts | jq '[.[] | select(.src != "")] | length'
```
...
hubcap scripts | jq --arg o "$ORIGIN" '[.[] | select(.src != "" and (.src | startswith($o) | not))]'
```
